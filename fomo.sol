...
contract WJFomo {
    ...
    function transferNotify(address user) external {
        require(msg.sender == address(hunter), "permission denied");
    ...
    function swap() public {
        ...
        inSwap = true;
        uint256 tokenAmount = hunter.balanceOf(address(this));
        if (tokenAmount == 0) {
            inSwap = false;
            return;
        }
        address[] memory path = new address[](2);
        path[0] = address(hunter);
        path[1] = address(usdt);

        hunter.approve(address(uniswapV2Router), tokenAmount);
    ...
    function buyAndBurn() public {
        ...
        // burn token
        uint256 tokenBalance = hunter.balanceOf(address(this));
        if (tokenBalance > 0) {
            hunter.transfer(blackHole, tokenBalance);
        }
    }
